...
mod tests {
    ...
    const HEADER: &[u8] = b"// (c) header\n";

    #[test]
    fn classify_covers_match_offender_generated_and_unreadable() {
        assert_eq!(
            classify(Ok(b"// (c) header\nfn main() {}\n".to_vec()), HEADER),
            Classification::Match
        );
        assert_eq!(
            classify(Ok(b"fn main() {}\n".to_vec()), HEADER),
            Classification::Offender
        );
        assert_eq!(
            classify(
                Ok(b"// @generated by tool\nfn main() {}\n".to_vec()),
                HEADER
            ),
            Classification::Generated
        );
        assert_eq!(
            classify(Err(std::io::Error::other("permission denied")), HEADER),
            Classification::Offender
        );
    }

    #[test]
    fn generated_marker_only_counts_inside_the_leading_window() {
        let within_window = b"a\nb\nc\nd\n// @generated\n";
        assert!(is_generated(within_window));
        let past_window = b"a\nb\nc\nd\ne\n// @generated\n";
        assert!(!is_generated(past_window));
    }

    #[test]
    fn default_globs_cover_the_standard_source_roots() {
        let globs = default_globs();
        for root in ["src/", "tests/", "examples/", "benches/"] {
            assert!(
                globs.iter().any(|g| g.starts_with(root)),
                "missing glob for {root}"
            );
        }
    }
}
