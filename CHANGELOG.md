...
## [0.7.1](https://github.com/Kuenlun/lockpick/compare/v0.7.0...v0.7.1) - 2026-09-20
...
- *(coverage)* inspect reports without rerunning tests ([#96](https://github.com/Kuenlun/lockpick/pull/96))
- replace dummy dependency imports with scoped exceptions ([#95](https://github.com/Kuenlun/lockpick/pull/95))
- respect project lint configuration ([#93](https://github.com/Kuenlun/lockpick/pull/93))
...
- pin check and release tool versions ([#97](https://github.com/Kuenlun/lockpick/pull/97))
...
- *(cli)* add `--coverage` to force the opt-in coverage gate with default 100% thresholds when no coverage table is configured ([#64](https://github.com/Kuenlun/lockpick/pull/64))
...
- **BREAKING:** Make the coverage gate opt-in instead of running it by default. Coverage now runs only when `[*.metadata.lockpick.coverage]` is present or `--coverage` is passed, and `cargo-llvm-cov` is required only while that gate is active ([#64](https://github.com/Kuenlun/lockpick/pull/64))
- **BREAKING:** Treat `--coverage` combined with `--skip coverage` or `--skip test` as a usage error with exit code `2`, including skips inherited from Cargo metadata ([#64](https://github.com/Kuenlun/lockpick/pull/64))
- *(coverage)* report `--skip coverage` as a no-op when coverage is not configured, and keep the explicit `--skip test` implies coverage notice when a configured coverage gate is disabled ([#64](https://github.com/Kuenlun/lockpick/pull/64))
- *(clippy)* allow `clippy::multiple_crate_versions` while keeping the rest of the `clippy::cargo` group enabled, avoiding failures caused by transitive duplicate dependency versions ([#64](https://github.com/Kuenlun/lockpick/pull/64))
